...
class ComponentRunResult:
    exit_code: int | None   # None = clean exit, 1 = UserException, 2 = AppError
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "logs": [
                {"level": log.level, "logger": log.logger_name, "message": log.message}
    ...
    def sanitize(self, result: ComponentRunResult) -> ComponentRunResult:
        ...
        for log in result.logs:
            ...
            sanitized_logs.append(
                CapturedLog(level=log.level, logger_name=log.logger_name, message=message)
            )
...
def compare_logs(
    ...
    Exit code: exact match required.
    Log messages: normalize both sides then compare lists with unified_diff.
    """
    if normalizers is None:
        normalizers = DEFAULT_NORMALIZERS

    exit_code_match = recorded.exit_code == replayed.exit_code
    ...
    if not exit_code_match:
        parts.append(
            f"Exit code mismatch: recorded={recorded.exit_code}, replayed={replayed.exit_code}"
        )
    if diffs:
